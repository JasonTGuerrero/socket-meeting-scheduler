...
string get_user_list_str(map<string, string> m) {
    string res;
...
string create_UDP_socket_and_receive_list() {
    ...
    struct sockaddr_in serv_addr;
    serv_addr.sin_family = AF_INET;
    serv_addr.sin_port = htons(SERVER_A_UDP_PORT);
    serv_addr.sin_addr.s_addr = inet_addr(HOSTNAME);
...
int main() {
    ...
    string users = get_user_list_str(m);
    ...
    // create_UDP_socket_and_receive_message(SERVER_A_UDP_PORT);
